/*globals define*/
define(["jquery","text!./MGOImageGridEmo.css"], 
				...
				externalimages: {
					...
					emotions: {
						...
						items: {
							emotionsTog : {
								ref : "qDef.IMGEMOTOG",
								label : "Turn on facial recognition and emotion analysis",
								type : "boolean",
								defaultValue : false
								},
							emotionsText : {
								label:"This requires an Emotion API key from MS cognitive services and for the images to be publically available online",
								component: "text"
								},
							emotionsLink : {
								label:"Get an Emotion Preview API key from the MS Cognitive Services site",
								component: "link",
...
//console.log(rowcount);
					...
					if((rowcount > 1) & (!grid1upDisplay)){
						...
						if(layout.qDef.mgoSinglePicControls){
							if((!killzoomcontrols) & (rowcount == 1) & (!layout.qDef.IMGEMOTOG)){
						...
						//check if measure added
							if((mymeasureCount==1) & (layout.qDef.SINGLEIMGMEASURE)){
								...
								// render measure
								html += '<span class="mgoMeasureSinglePic" style="z-index:65; height:auto; margin-left: 0px; '+ measBGtextCol +measBGtextPanel+'">'+ layout.qHyperCube.qMeasureInfo[0].qFallbackTitle +': <span style="'+colFlagTextColInsertM1+'">'+ meas1.qText+' ' +meas1Symbol+'</span></span>';
								...
								html += '<span class="mgoMeasureSinglePic" style="z-index:65; height:auto; margin-left: 0px; '+ measBGtextCol +measBGtextPanel+'">'+ layout.qHyperCube.qMeasureInfo[0].qFallbackTitle +': <span style="'+colFlagTextColInsertM1+'">'+ meas1.qText+' ' +meas1Symbol+'</span><br>'+ layout.qHyperCube.qMeasureInfo[1].qFallbackTitle +': <span style="'+colFlagTextColInsertM2+'">'+ meas2.qText+' ' +meas2Symbol+'</span></span>';
						...
						if(layout.qDef.SINGLEIMGHEADER){
							...
							if(grid1upDisplay){
								html += '<div class="mgoHeader" style="z-index:60"><a href="' + imgFolderLocation + dim.qText + '" target="blank"> <span class="lui-icon lui-icon--new-tab"></span> ' + dim.qText + '</a></div>';
							} else {	
								if(customSingleImageNameTog){
								html += '<div class="mgoHeader" style="z-index:60"><a href="' + customSingleImageLink + '" target="blank"> <span class="lui-icon lui-icon--new-tab"></span> ' + customSingleImageName + '</a>';
								
								} else {
									if((mydimensionCount == 2) & (layout.qDef.SINGLEIMGLINKPOPLINKSOURCE=="d1")){
										html += '<div class="mgoHeader" style="z-index:60"><a href="' + imgFolderLocation + dim.qText + '" target="blank"> <span class="lui-icon lui-icon--new-tab"></span> ' + dim.qText + '</a>';
									} else if((mydimensionCount == 2) & (layout.qDef.SINGLEIMGLINKPOPLINKSOURCE=="d2")) {
										html += '<div class="mgoHeader" style="z-index:60"><a href="' + dim2.qText + '" target="blank"> <span class="lui-icon lui-icon--new-tab"></span> ' + dim2.qText + '</a>';
									} else {
										html += '<div class="mgoHeader" style="z-index:60"><a href="' + imgFolderLocation + dim.qText + '" target="blank"> <span class="lui-icon lui-icon--new-tab"></span> ' + dim.qText + '</a>';
									};
								}
...
//console.log(rotateImgTurns);
			...
			if((mgoSinglePicModeActive) &(layout.qDef.IMGEMOTOG)){
				...
				var faceCount=0;
				var faceScoresCollected=[];
				
				var targEmotionTxtFaceID =$element.find(('.emotxtface'));
				...
				$element.find('.butEmoRequest').on('click', function() {
					...
					var analysisEMObut = $element.find(('.butEmoRequest'));
					analysisEMObut.html('ANALYSING...');
					analysisEMObut.css('background-color', '#CCCCCC');
					analysisEMObut.prop('disabled', true);
					...
					var emoBandtargWidth;
					var emoBandtargOffset;
					var emoImgScaled;
					...
					if(curZoomLevelArrParts[0]=='contain'){
						...
						if(bgImgWidth/containerWidth >= bgImgHeight/containerHeight){
							emoBandtargWidth = (bgImgWidth / (bgImgWidth/containerWidth)) + 'px';
							emoBandtargOffset = ((containerWidth-(bgImgWidth / (bgImgWidth/containerWidth)))/2)  + 'px';
							
						} else if(bgImgWidth/containerWidth < bgImgHeight/containerHeight){
							emoBandtargWidth = (bgImgWidth / (bgImgHeight/containerHeight)) + 'px';
							emoBandtargOffset = ((containerWidth-(bgImgWidth / (bgImgHeight/containerHeight)))/2)  + 'px';
							
						};

					} else if(curZoomLevelArrParts[0]=='100%'){

						emoBandtargWidth = '100%';
						emoBandtargOffset = 0  + 'px';


					} else if(curZoomLevelArrParts[1]=='100%'){
...
//console.log('width | scales ' + emoImgScaled + ' | ' + emoBandtargFaceOffsetL + ' , ' +emoBandtargFaceOffsetT);						
					...
					MSAPIKEY= layout.qDef.IMGEMOAPIKEY;
					

					if(MSAPIKEY.length > 4){
						...
						$(function() {
					        ...
					        $.ajax({
					            ...
					            beforeSend: function(xhrObj){
					                ...
					                xhrObj.setRequestHeader("Ocp-Apim-Subscription-Key",MSAPIKEY);
					            },
					            type: "POST",
					            // Request body
...
//console.log('a '+data.length);
					            ...
					            if(faceCount > 0){
					            	//first face
						            $.each(emoList, function ( key, emo  ) {
						            	var emoBand= $element.find((".emo"+emo));
						            	var emoBandText= $element.find((".emo"+emo+"txt"));
						            ...
						            targEmotionBox.css('width', emoBandtargWidth);
									targEmotionTxt.css('width', emoBandtargWidth);
									targEmotionBox.css('left', emoBandtargOffset);
									targEmotionTxt.css('left', emoBandtargOffset);

									targEmotionTxtFaceID.text('Emotion scores for Face 1');

									var targEmotionFacesBoxes = "";

									$.each(data, function ( key ) {	
										var faceEmoArr=[];
										$.each(emoList, function ( i, emo  ) {
											//var faceEmoArr=[];
											var faceEmoArrStr = parseFloat(data[key].scores[emo]);
											faceEmoArr.push(faceEmoArrStr);
										
							            									
										});
										faceScoresCollected.push(faceEmoArr);	
										var targFaceL= data[key].faceRectangle['left'];
										var targFaceT= data[key].faceRectangle['top'];
					        		...
					        		emotionErrorBox.html('No human faces detected');
...
//console.log("error");
				...
				$element.find('.emotionFaces').on('mouseover','.emoFaceID', function () {
					
					if((this.hasAttribute("data-value")) & (faceCount>1)) {
						var value = this.getAttribute("data-value");
						var targFaceToUse =$element.find(('.emoFacetoID'+value));
						$.each(emoList, function ( key, emo  ) {
						            	var emoBand= $element.find((".emo"+emo));
						            	var emoBandText= $element.find((".emo"+emo+"txt"));

						            	var emoScore = faceScoresCollected[value][key];
						            	emoBand.css('width', (emoScore * 99.9)+'%');
						            	emoBandText.css('font-size', (Math.round(emoScore * 50)+10)+'px');
						            	emoBandText.text((emoScore.toFixed(2) * 100) + '% '+emo);
									
						});
						targFaceToUse.css('background-color', 'rgba(255,255,255,0.3)');
						targEmotionTxtFaceID.text('Emotion scores for Face ' + (parseInt(value)+1));
					};
					

				});
				$element.find('.emotionFaces').on('mouseout','.emoFaceID', function () {
					...
					if((this.hasAttribute("data-value")) & (faceCount>1)) {
						...
						$.each(emoList, function ( key, emo  ) {
						            	var emoBand= $element.find((".emo"+emo));
						            	var emoBandText= $element.find((".emo"+emo+"txt"));
						            	var emoScore = faceScoresCollected[0][key];
						            	emoBand.css('width', (emoScore * 99.9)+'%');
						            	emoBandText.css('font-size', (Math.round(emoScore * 50)+10)+'px');
						            	emoBandText.text((emoScore.toFixed(2) * 100) + '% '+emo);
									
						});
						targFaceToUse.css('background-color', 'transparent');
						targEmotionTxtFaceID.text('Emotion scores for Face 1');
					};
